...
key.set('n', '<C-h>', '<C-w>h', { desc = 'Move to left split' })
key.set('n', '<C-j>', '<C-w>j', { desc = 'Move to down split' })
key.set('n', '<C-k>', '<C-w>k', { desc = 'Move to up split' })
key.set('n', '<C-l>', '<C-w>l', { desc = 'Move to right split' })
...
key.set('n', '<leader>fm', builtin.marks, { desc = 'Browse marks' })
key.set('n', '<leader>fd', builtin.diagnostics, { desc = 'Browse diagnostics' })
key.set('n', '<leader>fr', builtin.resume, { desc = 'Resume last Telescope picker' })

-- Quickfix
key.set('n', '<leader>qo', function()
  local wins = vim.fn.filter(vim.fn.getwininfo(), 'v:val.quickfix')
  if #wins > 0 then vim.cmd('cclose') else vim.cmd('copen') end
end, { desc = 'Toggle quickfix list' })
key.set('n', '<leader>qn', '<CMD>cnext<CR>', { desc = 'Next quickfix entry' })
key.set('n', '<leader>qp', '<CMD>cprev<CR>', { desc = 'Previous quickfix entry' })
...
key.set('n', '<leader>gr', builtin.lsp_references, { desc = 'LSP: Find references' })
key.set('n', '<leader>gi', vim.lsp.buf.implementation, { desc = 'LSP: Go to implementation' })
key.set('n', '<leader>gt', vim.lsp.buf.type_definition, { desc = 'LSP: Go to type definition' })
